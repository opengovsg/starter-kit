...
import { upsertUserAndAccountByEmail } from '../user.service'
...
describe('user.service', () => {
  ...
  describe('upsertUserAndAccountByEmail', () => {
    ...
    it('should return existing user when upserting with same email', async () => {
      ...
      // First upsert
      const user1 = await upsertUserAndAccountByEmail(email)

      // Second upsert with same email
      const user2 = await upsertUserAndAccountByEmail(email)
    ...
    it('should throw error for invalid email address', async () => {
      ...
      await expect(upsertUserAndAccountByEmail(invalidEmail)).rejects.toThrow(
    ...
    it('should throw error for group email addresses', async () => {
      ...
      await expect(upsertUserAndAccountByEmail(groupEmail)).rejects.toThrow(
        'Invalid email address',
      )
    })
    ...
    it('should handle multiple different users', async () => {
      ...
      const users = await Promise.all(
        emails.map((email) => upsertUserAndAccountByEmail(email)),
      )
